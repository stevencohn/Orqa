...
namespace River.Native
{
	...
	public class WowRegistry
	{
		...
		private static bool is64BitMachine = false;
		...
		static WowRegistry ()
		{
			...
			try
			{
				ManagementObjectSearcher searcher
					= new ManagementObjectSearcher("SELECT AddressWidth FROM Win32_Processor");

				ManagementObjectCollection collection = searcher.Get();

				foreach (ManagementObject obj in collection)
				{
					is64BitMachine = (UInt16)(obj["AddressWidth"]) == (UInt16)64;
					break;
				}
			}
			catch (Exception)
			...
			else
			{
				...
				if ((keyName == null) || keyName.Equals(String.Empty))
				{
					switch (hive)
					{
						...
						case RegistryHive.DynData: regKey = Registry.DynData; break;
