...
namespace River.Orqa.Controls.Docking
{
	...
	public class DockingManager
	{
		...
		public void ValuesFromState (State newState, out DockStyle dockState, out River.Orqa.Controls.Common.Direction direction)
		{
			switch(newState)
			{
				case State.Floating:
					dockState = DockStyle.Fill;
					direction = Common.Direction.Vertical;
					break;
				case State.DockTop:
					dockState = DockStyle.Top;
					direction = Common.Direction.Horizontal;
					break;
			...
			if (_autoResize)
			{
				...
				if ((f == null) || ((f != null) && (f.WindowState != FormWindowState.Minimized)))
				{
					if ((inner.Width < 0) || (inner.Height < 0))
					{
						...
						if (inner.Width < 0)
							ResizeDirection(-inner.Width, zcLeft, zcRight, Common.Direction.Horizontal);

						if (inner.Height < 0)
							ResizeDirection(-inner.Height, zcTop, zcBottom, Common.Direction.Vertical);
		...
		protected void ResizeDirection (int remainder, ZoneCollection zcAlpha, ZoneCollection zcBeta, River.Orqa.Controls.Common.Direction dir)
		{
			bool alter;
			int available;
			int half1, half2;
			...
			{
				if (dir == Common.Direction.Horizontal)
				{
